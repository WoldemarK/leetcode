...
public class Main {
    ...
    NotificationFilter.UserSettingsService userSettingsService = userIds -> {
        Map<Long, NotificationFilter.UserPreferences> prefs = new HashMap<>();
        for (Long id : userIds) {
            if (id == 100L) {
                // Пользователь 100: разрешены только EMAIL и PUSH, заблокирован отправитель 50
                prefs.put(id, NotificationFilter.UserPreferences.builder()
                        .allowedNotificationTypes(Set.of(NotificationFilter.NotificationType.EMAIL,
                                NotificationFilter.NotificationType.PUSH))
                ...
                prefs.put(id, NotificationFilter.UserPreferences.builder()
                        .allowedNotificationTypes(Set.of(NotificationFilter.NotificationType.EMAIL,
                                NotificationFilter.NotificationType.SMS,
                                NotificationFilter.NotificationType.PUSH))
    ...
    NotificationFilter.NotificationHistoryService notificationHistoryService = (userIds, since) -> {
        // Имитируем, что пользователю 100 уже отправляли уведомление ID=1 за последние 24 часа
        Instant now = Instant.now();
        return List.of(
                new NotificationFilter.SentNotificationRecord(
                        1L,
                        100L,
    ...
    List<NotificationFilter.Notification> notifications = Arrays.asList(
            // Уведомление 1: уже отправлялось пользователю 100 → должно быть отфильтровано (дубль)
            new NotificationFilter.Notification(1L, NotificationFilter.NotificationType.EMAIL, "Hello!", 100L),

            // Уведомление 2: SMS для пользователя 100 → запрещён канал → отфильтровано
            new NotificationFilter.Notification(2L, NotificationFilter.NotificationType.SMS, "Hi!", 100L),

            // Уведомление 3: EMAIL от отправителя 50 → заблокирован → отфильтровано
            new NotificationFilter.Notification(3L, NotificationFilter.NotificationType.EMAIL, "Spam?", 100L),

            // Уведомление 4: PUSH для пользователя 200 → всё разрешено → пройдёт
            new NotificationFilter.Notification(4L, NotificationFilter.NotificationType.PUSH, "Welcome!", 200L),

            // Уведомление 5: EMAIL для пользователя 200 → пройдёт
            new NotificationFilter.Notification(5L, NotificationFilter.NotificationType.EMAIL, "News", 200L)
    );

    // === 4. Запускаем фильтрацию (отправитель = 50) ===
    Long senderId = 50L;
    List<NotificationFilter.Notification> result = filter.filter(senderId, notifications);

    // === 5. Выводим результат ===
        System.out.println("Отфильтрованные уведомления (готовые к отправке):");
        for (NotificationFilter.Notification n : result) {
